...
export function ItemPhaseScreen() {
  ...
  return (
    <div
      className="w-full h-full flex flex-col pt-16 overflow-hidden"
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 scrollbar-hide flex flex-col items-center py-2">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {currentTeam.items.filter((i) => i.type === 'shield').map((item) => (
                <div key={item.id} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#f0fdf4] border border-[#bbf7d0] text-[#16a34a] text-sm font-body">
                  <span>🛡️</span>
      ...
      {/* Fixed footer — ÜBERSPRINGEN always visible when team has items */}
      {currentTeam && hasUsable && (
        <div className="flex-shrink-0 bg-white border-t border-[#e5e7eb] p-4 flex justify-center">
          <Button size="lg" variant="secondary" onClick={() => { clearInterval(timerRef.current); advanceItemPhaseTeam() }}>
            ÜBERSPRINGEN →
          </Button>
        </div>
      )}
